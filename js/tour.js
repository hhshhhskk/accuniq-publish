...
window.addEventListener("load", function () {
  ...
  function makeTourSlideHtml(_data) {
    ...
    for (let i = 0; i < TourRes.total; i++) {
      let price = TourRes.tour_slide[i].price
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    ...
    <div class="swiper-slide">
        <div class="tour-slide-item">
            <a href="${TourRes.tour_slide[i].url}">
                <img src="${TourRes.tour_slide[i].file}" alt="${TourRes.tour_slide[i].url}" />
            </a>
            <div class="tour-slide-info" >
              <div class="tour-itemBadge">
              ${TourRes.tour_slide[i].babge}
              </div>
              <div class="tour-itemBenefit">
              ${TourRes.tour_slide[i].benefit}
              </div>
              <div class="tour-itemName">
              ${TourRes.tour_slide[i].name}
              </div>
